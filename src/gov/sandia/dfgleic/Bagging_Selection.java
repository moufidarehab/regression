 ...
 */
public class Bagging_Selection extends Configured implements Tool {
    private static final Logger sLogger = Logger.getLogger(Bagging_Selection .class);



    public static void main(String args[]) throws Exception {
        // Let ToolRunner handle generic command-line options
        int res = ToolRunner.run(new Configuration(), new Bagging_Selection(), args);
    ...
    public int run(String[] args) throws Exception {
        ...
        for (int stage=0; stage<stages.length; ++stage) {
            ...
            JobConf conf = new JobConf(getConf(), Bagging_Selection.class);
            ...
            extends MapReduceBase
    {
        ...
        public String SelectONE_Backward(Hashtable<String, DenseVector> S, Hashtable<String, DenseVector> H_S, Hashtable<String, Double> Somm_Residual_ALL) throws IOException
        {
            String key_select = "";
            if (S.size() == 0) //cas du début de la selection
            {
        ...
        public void Evaluate_Models(DenseMatrix OutWeight)   throws IOException
        {
            ...
            Text KeyH = new Text();
            KeyH.set(new Text("H_").toString().concat(String.valueOf(row_id)));
            output.collect(KeyH, encodeTypedBytesMatrix(OutWeight));
    ...
    public static class BaggingReducer
        ...
        public void reduce(Text key, Iterator<TypedBytesWritable> values,
            ...
            while (values.hasNext()) {
                ...
                {
                    DenseMatrix H = decodeTypedBytesMatrix(values.next());
                    for (int i=0;i<H.numRows();i++)
                    {
                        String newKey = key.toString().concat("_" + String.valueOf(i));
                        DenseVector Model = new DenseVector(H.numColumns());
                        for (int j=0;j<H.numColumns();j++)
                        {
                            Model.set(j,H.get(i,j));
                        }
                ...
                else if (key.toString().startsWith("Y_Residual_"))  //recuperation de la somme des residuals
                {
                    ...
                    {
                        String newKey = key.toString().concat("_" + String.valueOf(j));
                        Somm_Residual_ALL.put(newKey,Y_Residual[j]);
                        //output.collect(new Text(newKey), encodeTypedBytes(Y_Residual));
                    }
                ...
                {
                    DenseMatrix Y_Prob = decodeTypedBytesMatrix(values.next());
